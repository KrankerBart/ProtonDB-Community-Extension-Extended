...
async function addLineToItem(item) {
  ...
  try {
    const data = await new Promise((resolve, reject) => {
      browserAPI.runtime.sendMessage({ contentScriptQuery: 'queryGame', appId }, (response) => {
...
async function addIconToItem(item, isItemInSearchBox = false) {
  ...
  try {
    const data = await new Promise((resolve, reject) => {
      browserAPI.runtime.sendMessage({ contentScriptQuery: 'queryDeckStats', appId }, (response) => {
        if (browserAPI.runtime.lastError) {
          reject(browserAPI.runtime.lastError)
          return
        }
...
function getNewItems() {
  ...
  return Array.from(document.querySelectorAll(selector)).filter(
    (item) => !items.has(item) && !hasAnyClass(item, ClassNamesToCheck) && !hasAnyParentClass(item, ClassNamesToCheckInParent),
  )
}
