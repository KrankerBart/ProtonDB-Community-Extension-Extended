...
async function initializeExtension() {
  try {
    ...
    const appOtherSiteInfo = document.querySelector('.apphub_OtherSiteInfo')
    appOtherSiteInfo.setAttribute('data-ds-appid', appId)

    // Only add icon if not in excluded classes
    if (!hasAnyClass(appHeaderContent, ClassNamesToCheck) && !hasAnyParentClass(appHeaderContent, ClassNamesToCheckInParent)) {
      // addIconToItem(appHeaderContent)
      addIconToItem(appOtherSiteInfo)
    }

    try {
      // Get game data
      const gameData = await new Promise((resolve, reject) => {
        browserAPI.runtime.sendMessage({ contentScriptQuery: 'queryGame', appId: appId }, (response) => {
      ...
      const platformData = await new Promise((resolve, reject) => {
        browserAPI.runtime.sendMessage({ contentScriptQuery: 'queryPlatforms', appId }, (response) => {
          if (browserAPI.runtime.lastError) {
            reject(browserAPI.runtime.lastError)
            return
          }
    ...
    gameLinks.forEach((item) => {
      if (!items.has(item) && !hasAnyClass(item, ClassNamesToCheck) && !hasAnyParentClass(item, ClassNamesToCheckInParent)) {
